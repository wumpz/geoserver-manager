...
/**
 *
 * Note on adding multiple available styles to the GSLayerEncoder: - to run the testIntegration(), 2 clones of the
 * "point" style, named "point2" and "point3" have to be created.
 *
...
public class GSFeatureEncoderTest extends GeoserverRESTTest {
   ...
   * http://docs.geoserver.org/latest/en/user/data/database/sqlview.html#parameterizing-sql-views (states shapefile -
   * available in testdata/states.zip) - create a postgis db - import the states shapefile (using shp2pgsql or Postgis
   * shapefile uploader) - In Geoserver, create a postgis datastore for this DB, with the name "statesdb"
   *
   * @throws IOException
   *
   */
